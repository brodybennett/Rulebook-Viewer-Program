...
- Current state: Early-draft rules with scaffolded Markdown files; viewer MVP and sanity tooling already working for browsing and link checks.
- What makes this unique: Wiki-link and anchor-aware workflow with optional registry support; single-file, read-only Flask viewer that runs locally without touching your files.
...
- Adding or confirming anchors in `tag-registry.yml` when you choose to keep a registry file in the repo.
- Running the viewer and sanity checker on Windows/macOS/Linux to report broken links, layout bugs, or confusing UX.
- Proposing structure tweaks to the navigation order under `draft/` so new players can onboard faster.

## Start in 5 minutes
1) Install Python 3.10+.
2) Install deps: `pip install flask pyyaml mistune`.
3) From the repo root, launch the viewer: `python "VSCODE MD Files/rulebook_viewer.py" --source "VSCODE MD Files"` (defaults to `draft/`; add `--no-browser` if you do not want auto-open).
