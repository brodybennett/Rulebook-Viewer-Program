...
# Chapter 3: Attributes {#chapter-3-attributes}

## Attribute Generation {#chapter-3-attributes-attribute-generation}
...
- **Point-Buy:** You have **32 points** to increase Attributes, but no Attribute may be lower than **2**.  
  [[UNCLEAR: Point-buy starting values and the point cost to increase an Attribute are not stated.]]

## Attribute Rating Scale {#chapter-3-attributes-attribute-rating-scale}
...
## Attribute List {#chapter-3-attributes-attribute-list}
...
## Using Attributes in Checks {#chapter-3-attributes-using-attributes}
...
## Changing Attributes {#chapter-3-attributes-changing-attributes}
...
## Derived Statistics {#chapter-3-attributes-derived-statistics}

### Spirituality {#chapter-3-attributes-spirituality}
...
- `(initial WIL + initial INT) + (number of Sequences you have advanced) × (your current/final INT)`
...
- Become a Sequence 9 Beyonder; the potion gives **+1 INT**, so INT becomes **7** → Spirituality is `(4 + 6) + (1 × 7)`.
- Become a Sequence 8 Beyonder; gain another **+1 INT**, so INT becomes **8** → Spirituality is `(4 + 6) + (2 × 8)`.

[[UNCLEAR: The text alternates between multiplying by “number of Sequences you have advanced” versus “current Sequence number”; the worked examples follow advancement count, while the sheet instruction references the displayed Sequence (e.g., 7 for Sequence 7).]]
...
> **GM Note:** If you are using the auto character sheet from the [[MISSING REF: Companion Supplemental Rules]], the auto sheet will handle Spirituality and Vitality. Remember to select your current Sequence number in the upper-left corner, or calculations will be affected (example given: Sequence 7 → select “7”). [[MISSING REF: Auto Character Sheet]]

### Sanity / Rationality {#chapter-3-attributes-sanity-rationality}
...
### Vitality {#chapter-3-attributes-vitality}
...
- Each time you advance one Sequence, increase Vitality by an amount equal to your **current Constitution** at the time of advancement, unless otherwise specified by the Sequence (see [[The Beyonder System]]).

**Key distinction from Spirituality (as written):**
- A potion’s increase to Constitution applies **once** when consuming the potion. It does **not** continue to affect prior Sequence contributions after you advance again.
...
#### High-Sequence Vitality Calculation {#chapter-3-attributes-high-sequence-vitality}
...
### Death and Unconsciousness Checks {#chapter-3-attributes-death-and-unconsciousness-checks}

If your Vitality drops to **0**, you immediately fall **unconscious**.

- Each round, you must make a Constitution (CON) check with **Difficulty 10**.
...
- Make a Constitution (CON) check with **Difficulty 10 + (damage taken × 2)** to avoid death.
...
## Combat and Action Statistics {#chapter-3-attributes-combat-and-action-statistics}

### Movement {#chapter-3-attributes-movement}
...
### Defense Difficulty {#chapter-3-attributes-defense-difficulty}
...
### Attribute Damage Bonus {#chapter-3-attributes-attribute-damage-bonus}
...
### Common Damage Dice and Vitality {#chapter-3-attributes-common-damage-dice-and-vitality}
...
## Carrying Items {#chapter-3-attributes-carrying-items}

Some Beyonder Abilities depend on the number of props/items you carry (examples given: a [[MISSING REF: Magician Pathway]]’s matches/paper figurines; a [[MISSING REF: Mystery Pryer Pathway]]’s spellcasting materials). The number of items you carry is subject to quantity limits.

### Item Carry Slots {#chapter-3-attributes-item-carry-slots}

Determine how many items you can carry:

1) **Theoretical Item Carry Slots:** `Strength (STR) × 2`  
...
3) **Items Exceeding Carry Max:**  
   If your actual carry slots ＞ theoretical carry slots, and your items are indeed already filling your slots, then for each **1** slot of items exceeding your theoretical item slots:
...
### Pocket Counts by Clothing {#chapter-3-attributes-pocket-counts-by-clothing}
...
### Hidden Pocket {#chapter-3-attributes-hidden-pocket}

**Hidden Pocket:** Items in hidden pockets are not considered valid selectable targets for checks such as Sleight of Hand, etc.

- However, the [[MISSING REF: Thief Pathway]] can, by default, use excellent Investigation ability to select objects in your hidden pockets as valid selectable targets.

If you are unconscious and someone searches your body:

- They must proactively request an Investigation check at **Difficulty 15** to discover hidden pockets.
...
### Backpacks That Provide Extra Item Slots {#chapter-3-attributes-backpacks-that-provide-extra-item-slots}
...
### Reputation Requirements for Different Clothing {#chapter-3-attributes-reputation-requirements-for-different-clothing}
...
### Self/Commissioned Upgrades to Increase Item Slots {#chapter-3-attributes-self-commissioned-upgrades-to-increase-item-slots}
...
- Crafting skill **Proficient:** Difficulty 15 Crafting check; add **+2** item slots to one clothing/backpack; may choose **1** hidden pocket
- Crafting skill **Advanced:** Difficulty 15 Crafting check; add **+3** item slots to one clothing/backpack; may choose **2** hidden pockets
- Crafting skill **Expert:** Difficulty 20 Crafting check; add **+4** item slots to one clothing/backpack; may choose **3** hidden pockets
- Crafting skill **Scholar:** Difficulty 25 Crafting check; add **+5** item slots to one clothing/backpack; may choose **4** hidden pockets
- Crafting skill **Master:** Difficulty 30 Crafting check; add **+6** item slots to one clothing/backpack; may choose **5** hidden pockets
...
### Item Slots Occupied by Different Item Sizes {#chapter-3-attributes-item-slots-occupied-by-different-item-sizes}
