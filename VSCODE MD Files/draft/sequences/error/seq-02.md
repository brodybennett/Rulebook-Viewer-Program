...
- **Advancement Ritual:** Deviating from the established development of a major event enough to be recorded in history. *(Unofficial ceremony.)*
...
dice:
  ...
  heal_roll: null
  effect_roll: null
  notes: Check roll maps the skillful-hands contest vs Physical Defense for Trojan Horse targeting.
scaling:
- when: steal_identity_mode
  changes:
    effect_note: Identity theft lasts 2 hours; observers can detect incongruity with Intuition appraisals (DV 15/20).
- when: steal_cognition_mode
  changes:
    effect_note: Target loses self-cognition temporarily as described.
- when: steal_characteristics_mode
  changes:
    effect_note: Target must be voluntary or helpless and one Sequence lower.
- when: grafting_fate_mode
  changes:
    effect_note: Fate grafting swaps effects for 1 round with transfer caveats.
- when: steal_fate_mode
  changes:
    effect_note: Fate theft is permanent and typically requires a detached worm of time for helplessness.
tags:
- ritual
- detection
- defense
...
- **Use:** A Casting Action that attacks the opponent's **Physical Defense** "with skillful hands."
- **Effect:** You can start to steal other people's identity, destiny, self-knowledge, and related aspects, using one of the following options as appropriate:

#### Steal Identity

- **Effect:** You steal the target's identity, making others think that you are the target.
- **Duration:** 2 hours.
- **Detection:**  
...
- **Effect:** You can steal the target's **Extraordinary Characteristics**.
- **Target requirements:** The target must be either:
  - voluntary, **or**
  - in a helpless state (and one **Sequence** lower than you).

#### Grafting Fate

- **Effect:** This is also a kind of stealing. You change the opponent's fate to yours, and yours to theirs.
- **Duration:** 1 round.
- **Exchange of special effects:**  
  - During the grafting, the special effects you have on the opponent take effect on the opponent, and the opponent's special effects take effect on you.  
  - Special effects that were applied before the grafting will be transferred back if the round ends, but will be borne by the grafted person during the grafting process.  
  - If someone applies a strengthening or negative special effect to you during this process, then after the grafting is over, it will stay on the body of the grafted person.
- **Attacks:**  
  - An attack on you is regarded as an attack on the opponent.  
  - An attack on the opponent is regarded as an attack on you.
- **Clarification on "essence" of effects:**  
  - One case: the essence of a special effect is still on you, but you pass on the effect after it takes effect to others; therefore it is passed back after the grafting ends.  
...
- **Note:** Effects on targets with Sequence higher than yours can only be transferred in half.
- [[Special Effects]]

#### Steal Fate

- **Target requirements:** You choose a voluntary or helpless target and steal the other's fate.
- **Permanence:** After stealing the fate, you never need to return it.
- **Ongoing consequence:** From now on, all the good and bad that will happen to the target's fate in the future will be transferred to you.
- **Replacement effect:** You can completely replace the target-everything will think you are the target.
  - Parents, relatives, lovers, and friends will think you are the target, even though your appearance has not changed at all.
- **Effect on the stolen person:** The target becomes unable to have any communication with reality and has literally lost their destiny:
  - No one will notice them.
  - No matter what actions they take, no one will notice.
  - In the end, they can only die little by little in a corner, and may even turn into a corpse without anyone noticing.
- **Who can perceive the incongruity:** Only beings of the same high level can perceive the incongruity of fate, perceive something wrong, or perceive the truth.
...
- Stealing fate and turning others into clones are not the same:
  - Stealing a person's fate does not turn them into a doppelganger; it turns them into an unlucky person who still exists, but no one notices what they do.
  - Whatever you do can be regarded as them, and whatever you do will be regarded as them.
- These abilities are often used together, which can create the misunderstanding of "stealing fate = creating avatar."
- Creating a clone is creating a clone; stealing a fate is stealing a fate.
  - If you turn a person into a clone, that is still "parasitic": their fate is still in their body and has not been transferred to your body. This is the difference.
- Practical warning: stealing fate means the fate will belong to you whether good or bad in the future; stealing fate is not necessarily a good thing. It is more realistic to create a clone to "take over" fate.  
  > **GM Note:** This paragraph contains guidance on risk/choice; mechanics above still apply exactly as written.
- If you make the parasite into a clone:
  - The clone's ability depends on itself, but it can use your ability to a certain extent, which usually can only reach the level within **Sequence** 5.
...
dice:
  ...
  heal_roll: null
  effect_roll: null
  notes: Skill test resolves the initial parasitism attempt vs Willpower Defense.
scaling:
- when: demigod_target_evasion_window
  changes:
    effect_note: Demigod targets can attempt a DV 15 Intuition appraisal to detect the worm and respond immediately.
- when: same_level_or_one_level_lower_target
  changes:
    effect_note: Parasite entry is difficult against same or one-level-lower targets.
- when: deep_parasite_trigger
  changes:
    effect_note: On success, you may immediately convert to a deep parasite as a free action.
tags:
- ritual
- defense
...
- **Effect:** You detach a Worm of Time, which parasitizes targets through shadows that are hard to see.
  - You separate a Worm of Time and let it imperceptibly reach the target's side, and immediately perform a parasite instead of you.
  - When the Full-Round Action is over, the Worm of Time is considered to have arrived at the target's side.
  - At that time, conduct a skill test against the target's **Willpower Defense**; on success, it is considered to have entered a shallow parasitic state.
- > **GM Note:** The Full-Round Action implies you may want to talk to delay time and divert the opponent's attention so they do not become aware of your action.
...
- You can use verbal confrontation to divert the opponent's attention; once successful, it can have the following effects:
  ...
  - If the opponent's remaining spiritual value is 10 or below, and they are weak and helpless: the **Intuition (INT)** test fails by default and parasitism is basically guaranteed to succeed. Only the same level has a chance to resist with free actions.
- [[Fool]]
- [[Spectator]]
- [[White Tower]]
- [[Secret Peer]]
...
- A deeply parasitized target still has their own consciousness and can see, hear, and feel, but the body already belongs to you completely.
- The target is like a toy that can only be manipulated by you.
- You can always monitor their thoughts:
  - Their thoughts can only stay at the superficial level and cannot directly dig out secrets.
...
- **Effect:** You can make the target's fate go wrong for a certain period of time, achieving one of the following results:
...
dice:
  ...
  heal_roll: null
  effect_roll: null
  notes: Base check maps the Intuition appraisal; Library-based appraisal is captured in scaling.
scaling:
- when: using_library_check
  changes:
    check_roll: 1d20 + @attr.edu + @skill.library_use
    effect_note: Library check replaces Intuition for Insight of Destiny.
- when: dv_based_on_target_sequence
  changes:
    effect_note: Difficulty Value equals (30 - target sequence level).
tags:
- divination
...
- **Effect (check):** Pass a Library or **Intuition (INT)** check of **Difficulty Value** (30 - the target's sequence level).
...
- **Limits (demigod-and-above involvement):**
  - If the fate involves things above demigods, you can only feel "abnormal" at most for those contents.
  - If a demigod is right in front of you, you can gain insight into their general experience and education as above, but things above other demigods will still be covered up unless you get more clues.
- **Repeat use:** Reusing the skill will usually give the same result unless you acquire new material.
...
- **Detecting specific states (without full attribution):**
  - You can discover that a target is a Secret Puppet, but you cannot directly know who the Secret Puppet's owner is.
  - You can only see that they became a Secret Puppet at a certain moment.
  - You can find that a person influenced by the audience's psychology has had their destiny shift, but you cannot find hidden parasites.
