...
- **Effect:** You deceive supernatural powers and the laws of nature to defraud the laws of nature. This ability can do the following:
  ...
  - **Create a False Resurrection Point:** If you need to be resurrected and the original resurrection point cannot be used, you can choose a place with your fresh body tissue (the body tissue must be properly preserved) and change it to your new resurrection point. [[LINK LATER: Resurrection Point | type=term | hint=original vs new resurrection location rule]]
  ...
  - **Connect Two Portals with the Same Principle:** You can connect two portals with the same principle and treat them as intercommunicating gates.
    - It must be two portals with the same principle (example: two Apprentice Sequence 9 doors to the Spirit World). [[LINK LATER: Apprentice Pathway | type=pathway | hint=Sequence 9 portal to the Spirit World]] [[LINK LATER: Spirit World | type=location]]
    - The entrance and exit of the hidden space of Doorway Sequence 4 is also regarded as a kind of portal. [[LINK LATER: Doorway Pathway | type=pathway | hint=Sequence 4 hidden space portals]] [[LINK LATER: Hidden Space | type=location | hint=Doorway hidden space entrance/exit as portals]]
    ...
    - Based on the above principles, you cannot affect the position of a flame jump of an Extraordinary in the Path of the Fool, or carry out a teleportation flame jump with a flame jumping item, because you have no way of judging where an Extraordinary in the Path of the Fool will jump in the next second. [[LINK LATER: Path of the Fool | type=pathway]] [[LINK LATER: Flame Jump | type=mechanic | hint=teleportation-like jump]]
    ...
    - Because this is too instant and random, usually the ability of the decryption scholar is too late to decrypt the result of the jump position, so usually you can't consider doing this. [[LINK LATER: Decryption Scholar | type=sequence | hint=timing limitation reference]]
  - **Increase the Success Level of a Skill Identification:** Only for skills that have already been successful.
    ...
    - Every 5 values is a success level. Although in the example of “Chapter 2: Judgment” in the rule book, the success level is only 30, there are success levels higher than 30: 35, 40, 45. When any authentication value reaches one of the intervals, the fraud rule can make it just reach the next interval (example: 38 becomes 40). [[LINK LATER: Success Level | type=mechanic | hint=each 5 values is a level; can shift to next interval boundary]] [[LINK LATER: Chapter 2: Judgment | type=other | hint=rulebook example reference]]
    ...
    - The definition of “success” is:
      - In attack identification, the identification exceeds the opponent's defense Difficulty Value. [[LINK LATER: Attack Identification | type=mechanic | hint=check vs defense Difficulty Value]] [[LINK LATER: Defense Difficulty Value | type=mechanic | hint=opponent defense DV]]
      - In divination identification, it has reached the minimum value that divination can give a divination result (then it will be considered successful). [[LINK LATER: Divination Identification | type=mechanic | hint=minimum value to obtain a divination result]]
    - This ability doesn't change the base value of 1d20, only the final check, so it doesn't affect critical success or critical failure. [[LINK LATER: Critical Success | type=mechanic]] [[LINK LATER: Critical Failure | type=mechanic]]
...
>   [[LINK LATER: Fate Trojan Horse | type=mechanic | hint=Sequence 2 fate/identity transfer referenced here]]
>
...
>   [[LINK LATER: Spiritual Body Thread | type=term | hint=thread transfer/exchange mechanic reference]] [[LINK LATER: Astral Thread | type=term | hint=partial fate/thread affected by fraud]] [[LINK LATER: Destiny Trojan Horse | type=mechanic | hint=Sequence 2 transfer ability referenced]]
>
> **GM Note:** The RAW also references “Miracle Master of the Fool’s Path Sequence 2” and “judges” as examples of the broader world-law principle that what has already happened and results already achieved are not pursued or undone. [[LINK LATER: Miracle Master | type=sequence | hint=Fool’s Path Sequence 2 reference]] [[LINK LATER: Judge | type=pathway | hint=judges/rules reference in world-law example]]
