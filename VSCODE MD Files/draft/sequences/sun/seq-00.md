---
title: 'Sequence 0: Sun'
id: sun-seq-00
tags:
- pathway:sun
- sequence:0
---






# Sun Pathway: Sequence 0

## Sun

This **Sequence** embodies scorching light and extreme heat.

## Advancement

### Main Materials

- **Main Materials:** TBD.

### Auxiliary Materials

- **Auxiliary Materials:** TBD.

### Advancement Ritual

- **Advancement Ritual:** TBD.

## Extraordinary Abilities
### Attribute Gain

- **Attribute Gain:** Not explicitly specified in source (schema placeholder).


### Scorching Light

```yaml ability
id: sun-seq-00-scorching-light
name: Scorching Light
pathway: sun
sequence: 0
status: canonical
type: active
action: cast
cost: {}
roll: null
opposed_by: none
range: This light can illuminate the entire world and an entire galaxy. [[Illumination]]
target: designated target(s)
duration: instant
dice:
  check_roll: null
  damage_roll: null
  heal_roll: null
  effect_roll: "1"
  notes: No explicit dice; flames ignore normal fire resistance and damage reduction.
scaling: []
tags:
- utility
text: 'Effect: Emits scorching light. Targeting and range: This light can illuminate
  the entire world and an entire galaxy. [[Illumination]] Clarification: Treat this
  as planetscale illumination. Galaxy is lore only.'
```





- **Effect:** Emits scorching light.
- **Targeting and range:** This light can illuminate the entire world and an entire galaxy. [[Illumination]]
  - **Clarification:** Treat this as planet‑scale illumination. “Galaxy” is lore only.

- **Limits:** As described in this section's prose.


### Energy Sway

```yaml ability
id: sun-seq-00-energy-sway
name: Energy Sway
pathway: sun
sequence: 0
status: canonical
type: active
action: cast
cost: {}
roll: null
opposed_by: none
range: self
target: self
duration: instant
dice:
  check_roll: null
  damage_roll: null
  heal_roll: null
  effect_roll: null
  notes: No explicit dice expression in source text.
scaling: []
tags:
- utility
text: 'Effect: Can sway energy. [[Energy]] Clarification: You can redirect, dampen,
  or intensify light/heat/electrical energy within 1 km; exact effects are GMadjudicated.'
```





- **Effect:** Can sway energy. [[Energy]]
  - **Clarification:** You can redirect, dampen, or intensify light/heat/electrical energy within 1 km; exact effects are GM‑adjudicated.

- **Limits:** As described in this section's prose.


### Ultra-High Temperature Flames

```yaml ability
id: sun-seq-00-ultra-high-temperature-flames
name: Ultra-High Temperature Flames
pathway: sun
sequence: 0
status: canonical
type: active
action: cast
cost: {}
roll: null
opposed_by: none
range: self
target: self
duration: instant
dice:
  check_roll: null
  damage_roll: null
  heal_roll: null
  effect_roll: null
  notes: No explicit dice expression in source text.
scaling: []
tags:
- defense
- offense
text: 'Effect: Can create ultra-high temperature flames. [[Fire]] Clarification: These
  flames ignore normal fire resistance and damage reduction; only divine or equivalent
  protections apply.'
```





- **Effect:** Can create ultra-high temperature flames. [[Fire]]
  - **Clarification:** These flames ignore normal fire resistance and damage reduction; only divine or equivalent protections apply.

- **Limits:** As described in this section's prose.
